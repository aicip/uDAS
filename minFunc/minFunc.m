...
% funObj is the function, x0 is the initialized the value for w
...
% Check optimality of initial point %check if the gradients is vanish
if sum(abs(g)) <= tolFun
    exitflag=1;
...
for i = 1:maxIter
    ...
    switch method
        ...
        case PCG % Preconditioned Non-Linear Conjugate Gradient
            ...
            g_old = g;
            s_old = s;
        case LBFGS % L-BFGS % calculation is done here 
    ...
    elseif LS < 6
        ...
        funEvals = funEvals + LSfunEvals;
        x = x + t*d; % need to calculate the w here.
%         x = max(0,x); % this way will generate too many zeros. 
